...
APP_VERSION = "2.40.0-noexcel-v40-professional-ui"
...
def parse_file(path: Path, source_type: str) -> list[LogRecord]:
    ...
    if str(source_type).upper() == "WATERSYSTEM":
        wanted = re.compile(r"\b(MainState|CoolingStatus|Error)\b", re.I)
        for idx, line in enumerate(lines, start=1):
            msg = clean_message(line)
            if not msg or not wanted.search(msg):
                continue
            ...
            if ts:
                ...
                if ts:
                    last_ts = ts
            m = wanted.search(msg)
            key = m.group(1) if m else "WaterSystem"
            # Keep only the WaterSystem state/error information in the visible message.
            pos = msg.lower().find(key.lower())
            compact = msg[pos:].strip() if pos >= 0 else msg
            level = "ERROR" if key.lower() == "error" or "error" in compact.lower() else ""
            records.append(LogRecord(ts, "WATERSYSTEM", path.name, idx, level, key, compact, line.rstrip("\r\n")))
        if not records and fallback_dt:
            records.append(LogRecord(fallback_dt, "WATERSYSTEM", path.name, 0, "", "WATERSYSTEM", "File detected but no MainState/CoolingStatus/Error rows", ""))
        return records
    for idx, line in enumerate(lines, start=1):
        msg = clean_message(line)
        if not msg:
...
class DualLogViewer(QWidget):
    ...
    """
    SOURCES = ["Merged", "WS", "WaterSystem", "CGA", "CSA", "MRSERVER", "GESYS", "LAIS", "PSC", "Review", "Custom File"]
...
def _v40_main_build_ui(self):
    ...
    root.addWidget(title)
    subtitle = QLabel("v40 Professional UI: START-centered workflow, smart discovery, unified update, improved progress, WaterSystem parser.")
...
class SmartFileDiscoveryDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.enable_end = QCheckBox("End")
        self.end_date = QDateEdit(QDate.currentDate()); self.end_date.setCalendarPopup(True); self.end_date.setDisplayFormat("yyyy/MM/dd")
        for w in [self.enable_start, self.start_date, self.enable_end, self.end_date]:
            rg.addWidget(w)
        rg.addStretch(1)
        root.addWidget(range_box)
        for w in [self.enable_start, self.enable_end, self.start_date, self.end_date]:
            try:
                w.stateChanged.connect(self.apply_filter)
            except Exception:
                w.dateChanged.connect(self.apply_filter)
    ...
    def _set_detected_range_defaults(self):
        starts = [d.start_ts for d in self.discovered if d.start_ts]
        ends = [d.end_ts for d in self.discovered if d.end_ts]
        if starts:
            self.start_date.setDate(QDate(min(starts).year, min(starts).month, min(starts).day))
        if ends:
            self.end_date.setDate(QDate(max(ends).year, max(ends).month, max(ends).day))
    ...
    def populate_type_checks(self):
        ...
        for d in self.discovered:
            counts[d.log_type] = counts.get(d.log_type, 0) + 1
        preferred = ["WS", "WATERSYSTEM", "CGA", "CSA", "MRSERVER", "GESYS", "LAIS", "PSC", "REVIEW", "UNKNOWN"]
        for typ in [t for t in preferred if t in counts] + sorted(t for t in counts if t not in preferred):
            chk = QCheckBox(f"{typ} ({counts[typ]})")
            chk.setChecked(False)
            chk.stateChanged.connect(self.apply_filter)
